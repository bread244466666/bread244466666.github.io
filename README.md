...
<section id="box" class="game">
  ...
  <canvas id="boxGame" width="480" height="400"></canvas>
...
const tile=40;
let map,p,currentLevel=0;

// 50 unique glitch-free levels with equal boxes and goals
const sokobanLevels=[
`##########
#........#
#..P.....#
...
function startBoxGame(){
  currentLevel = currentLevel % sokobanLevels.length;
  const levelStr = sokobanLevels[currentLevel];
  map = levelStr.trim().split('\n').map(r=>r.split(''));
  for(let y=0;y<map.length;y++) for(let x=0;x<map[y].length;x++) if(map[y][x]==='P'){p={x,y}; map[y][x]='.';}
  document.getElementById('boxLevel').innerText='Level:'+(currentLevel+1);
  drawBox();
}
...
function checkWin(){
  ...
  setTimeout(()=>{ alert('Level Complete!'); currentLevel++; startBoxGame(); },100);
}

function drawBox(){
  bctx.clearRect(0,0,480,400);
